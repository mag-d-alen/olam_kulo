...
export class OnboardingService {
  ...
  async setItinerary(userId: string) {
    const supabase = this.authService.getAuthClient();
    const { data, error } = await supabase
      .from('user_itinerary')
      .insert({
    ...
    if (error)
      throw new PostgrestError({
        message: 'Error setting itinerary' + error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
      });
    return { message: 'Itinerary set successfully', data };
  }
  ...
  private async findIdOfExistingPlace(city: string, country: string) {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error)
      throw new BadRequestException('Error finding existing place' + error);
    return data;
  }

  private async createNewPlace(
    city: string,
    country: string,
    lat: number,
    lng: number,
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error)
      throw new BadRequestException('Error creating new place' + error.message);
    return data;
  }
}
